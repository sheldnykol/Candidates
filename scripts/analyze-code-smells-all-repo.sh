...
# Directories to analyze (all student work in src/)
ANALYZE_DIRS="src"

# Directories to always exclude (instructor materials and config)
EXCLUDE_DIRS="__tests__ test test_old Training-Files node_modules dist build coverage"

# Files to exclude (base files provided by instructor)
EXCLUDE_FILES="App.tsx main.tsx index.css App.css vite-env.d.ts"
...
echo "=================================================="
echo "📁 Analyzing: All of $ANALYZE_DIRS/ (student work)"
...
MISSING_KEYS=$(grep -rn "\.map(" $ANALYZE_DIRS --include="*.tsx" $EXCLUDE_PATTERN -A 2 2>/dev/null | grep -v "key=" | wc -l | tr -d ' ')
